...
def filter_genes_with_go(selected_genes, gene_to_go, min_terms=3):
    filtered = {}
...
def compute_information_content(go_to_genes, godag):
    total_genes = len(set(g for genes in go_to_genes.values() for g in genes))
    
    ic = {}
    for go_id in godag:
        if go_id in go_to_genes:
            freq = len(go_to_genes[go_id]) / total_genes
            ic[go_id] = -np.log(freq) if freq > 0 else 0
        else:
            ic[go_id] = 0
